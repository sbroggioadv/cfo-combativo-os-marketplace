...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando .cfo/ acima
- hook_state_load/save    → gerencia .cfo/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/.cfo/cfo-state.json."""
    start = start.resolve()
